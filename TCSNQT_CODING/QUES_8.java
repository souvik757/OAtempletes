 ...
 * Bob is going to bet today on horse riding. There are N horses listed in a sequence of 1 to N.
   ...
   As you are his best friend, he reached out to you for help,
   can you please find the length of the maximum continuous sequence of horses on which Bob can make a bet,
   and remember he will invest money less than K units. If there is more than one possible combination,
   ...
   There are two possible o sequences of length three whose total money for betting is less than 100, i.e.
   [50 20 20] (sum is 90) and [10 10 10] (sum is 30). Bob will choose randomly one sequence from these two.
   ...
   Bob will choose randomly any one horse. And the max length of the sequence will be 1.
   Constraints:
...
public class QUES_8 {
        ...
        solution_Ques_8 obj = new solution_Ques_8() ;
        int N = sc.nextInt() ;
        int K = sc.nextInt() ;
        int[] arr = new int[N] ;
        for (int i = 0 ; i< N ; i ++)
        ...
        System.out.println(obj.getMaxLength(arr,N,K)) ;
    }
}
class solution_Ques_8 {
    public int getMaxLength(int[] arr , int N , int K){
        int sum = 0 ;
        int s = 0 ;
